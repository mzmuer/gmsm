...
func (curve p256Curve) IsOnCurve(X, Y *big.Int) bool {
	...
	p256Square(&x3, &x)       // x3 = x ^ 2
	p256Mul(&x3, &x3, &x)     // x3 = x ^ 2 * x
	p256Mul(&a, &curve.a, &x) // a = a * x
	p256Add(&x3, &x3, &a)
	p256Add(&x3, &x3, &curve.b)

	p256Square(&y2, &y) // y2 = y ^ 2
...
func p256ScalarBaseMult(xOut, yOut, zOut *p256FieldElement, scalar *[32]uint8) {
	...
	for i := uint(0); i < 32; i++ {
		...
		for j := uint(0); j <= 32; j += 32 {
			...
			p256CopyConditional(zOut, &p256Factor[1], nIsInfinityMask)
...
func p256ScalarMult(xOut, yOut, zOut, x, y *p256FieldElement, scalar *[32]uint8) {
...
var p256Factor = []p256FieldElement{
...
func p256PointAdd(x1, y1, z1, x2, y2, z2, x3, y3, z3 *p256FieldElement) {
	...
	p256Square(&z12, z1) // z12 = z1 ^ 2
	p256Square(&z22, z2) // z22 = z2 ^ 2

	p256Mul(&z13, &z12, z1) // z13 = z1 ^ 3
	p256Mul(&z23, &z22, z2) // z23 = z2 ^ 3

	p256Mul(&u1, x1, &z22) // u1 = x1 * z2 ^ 2
	p256Mul(&u2, x2, &z12) // u2 = x2 * z1 ^ 2

	p256Mul(&s1, y1, &z23) // s1 = y1 * z2 ^ 3
	p256Mul(&s2, y2, &z13) // s2 = y2 * z1 ^ 3
	...
	p256Sub(&h, &u2, &u1) // h = u2 - u1
	p256Sub(&r, &s2, &s1) // r = s2 - s1

	p256Square(&r2, &r) // r2 = r ^ 2
	p256Square(&h2, &h) // h2 = h ^ 2

	p256Mul(&tm, &h2, &h) // tm = h ^ 3
	p256Sub(x3, &r2, &tm)
	p256Mul(&tm, &u1, &h2)
	p256Scalar(&tm, 2)   // tm = 2 * (u1 * h ^ 2)
	p256Sub(x3, x3, &tm) // x3 = r ^ 2 - h ^ 3 - 2 * u1 * h ^ 2

	p256Mul(&tm, &u1, &h2) // tm = u1 * h ^ 2
	p256Sub(&tm, &tm, x3)  // tm = u1 * h ^ 2 - x3
	p256Mul(y3, &r, &tm)
	p256Mul(&tm, &h2, &h)  // tm = h ^ 3
	p256Mul(&tm, &tm, &s1) // tm = s1 * h ^ 3
	p256Sub(y3, y3, &tm)   // y3 = r * (u1 * h ^ 2 - x3) - s1 * h ^ 3

	p256Mul(z3, z1, z2)
	p256Mul(z3, z3, &h) // z3 = z1 * z3 * h
}

func p256PointDouble(x3, y3, z3, x, y, z *p256FieldElement) {
	var s, m, m2, x2, y2, z2, z4, y4, az4 p256FieldElement

	p256Square(&x2, x) // x2 = x ^ 2
	p256Square(&y2, y) // y2 = y ^ 2
	p256Square(&z2, z) // z2 = z ^ 2

	p256Square(&z4, z)   // z4 = z ^ 2
	p256Mul(&z4, &z4, z) // z4 = z ^ 3
	p256Mul(&z4, &z4, z) // z4 = z ^ 4

	p256Square(&y4, y)   // y4 = y ^ 2
	p256Mul(&y4, &y4, y) // y4 = y ^ 3
	p256Mul(&y4, &y4, y) // y4 = y ^ 4
	p256Scalar(&y4, 8)   // y4 = 8 * y ^ 4

	p256Mul(&s, x, &y2)
	p256Scalar(&s, 4) // s = 4 * x * y ^ 2

	p256Dup(&m, &x2)
	p256Scalar(&m, 3)
	p256Mul(&az4, &p256.a, &z4)
	p256Add(&m, &m, &az4) // m = 3 * x ^ 2 + a * z ^ 4

	p256Square(&m2, &m) // m2 = m ^ 2

	p256Add(z3, y, z)
	p256Square(z3, z3)
	p256Sub(z3, z3, &z2)
	p256Sub(z3, z3, &y2) // z' = (y + z) ^2 - z ^ 2 - y ^ 2

	p256Sub(x3, &m2, &s)
	p256Sub(x3, x3, &s) // x' = m2 - 2 * s

	p256Sub(y3, &s, x3)
	p256Mul(y3, y3, &m)
	p256Sub(y3, y3, &y4) // y' = m * (s - x') - 8 * y ^ 4
}
